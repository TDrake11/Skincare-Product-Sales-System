...
namespace Skincare_Product_Sales_System_Application.Services.OrderService
{
    public class OrderService : IOrderService
    {
        ...
        public async Task AddOrderAsync(Order comment)
        {
            await _unitOfWork.Repository<Order>().AddAsync(comment);
            await _unitOfWork.Complete();
        }

        public async Task UpdateOrderAsync(Order comment)
        {
            _unitOfWork.Repository<Order>().Update(comment);
            await _unitOfWork.Complete();
        }
